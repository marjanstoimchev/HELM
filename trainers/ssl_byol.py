...
class SemiSupervisedBYOLModel(L.LightningModule):
    """
    ...
    def forward(self, x, y, u=None):
        ...
        return {
            'logits_h': logits_cls,
            'logits': logits_cls[:, :self.num_leaves],
    ...
    def _forward_eval(self, x, y):
        ...
        x_cls, patch_emb = self.backbone(x)
        logits_cls, loss_cls = self._compute_cls_loss(x_cls, y)
    ...
    def validation_step(self, batch, batch_idx):
        """Compute and log the validation loss (cls only, no BYOL)."""
